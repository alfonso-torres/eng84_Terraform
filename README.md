...
The third iteration is to get node app working with DB, it means to get `ip/posts` working like we have done with ansible.

For this we will have to create a new route table, a subnet and a security group for the db instance as well as the instance itself. That is, the same steps that we have done for the app but this time it will be private because we want to protect our database and only have access through the app.
